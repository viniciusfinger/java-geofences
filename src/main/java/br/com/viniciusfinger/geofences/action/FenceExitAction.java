...
@Slf4j
@Component
public class FenceExitAction implements Action {
    ...
    public void execute(FenceFeatureInterest fenceFeatureInterest, TelemetryDTO telemetryDTO, Telemetry lastTelemetry) {
        ...
        if (isExitingFence(fence, lastPosition, actualPosition)) {
            log.info("Device {} has entered fence id {}", telemetryDTO.getDeviceId(), fence.getId());

            Event entranceEvent = Event.builder().type(EventType.FENCE_EXIT)
    ...
    private boolean isExitingFence(Fence fence, Point lastPosition, Point actualPosition) {
